...
class Table:
    ...
    def _outer_join(self, table_right: Table, condition):
        '''
            [Usage] Join table (left) with a supplied table (right) where condition is met.
        '''
        # [1] Get columns and operator
        column_name_left, operator, column_name_right = self._parse_condition(condition, join = True)

        # [2] Try to find both columns, if you fail raise error
        try:
            column_index_left = self.column_names.index(column_name_left)
            column_index_right = table_right.column_names.index(column_name_right)
        except:
        ...
        # [3] Get the column names of both tables with the table name in front
        # ex. for left -> name becomes left_table_name_name etc
        left_names = [f'{self._name}_{colname}' for colname in self.column_names]
        right_names = [f'{table_right._name}_{colname}' for colname in table_right.column_names]

        # [4] Define the new tables name, its column names and types
        join_table_name = f'{self._name}_join_{table_right._name}'
        join_table_colnames = left_names+right_names
        join_table_coltypes = self.column_types+table_right.column_types
        join_table = Table(name=join_table_name, column_names=join_table_colnames, column_types= join_table_coltypes)

        # [5] count the number of operations (<,> etc)
        # outer join :  append the row in any case
        #               [*] (left, right) = (null, not null)
        #               [*] (left, right) = (not null, null)
        #               [*] (left, right) = (not null, not null)
        no_of_ops = 0
    ...
    def _sort_merge_join(self, table_right: Table, condition):
        ...
        # [5] count the number of operations (<,> etc)
        # we suppose that values are unique so that the sort-merge algorithm works correctly
        # sort-merge join :  
        #                   (step 1) sort both dataframes based on the columns

        #                   (step 2.1) Check if the rows match. If they do produce output row
        #                   (step 2.2) If the value in input A is smaller than the value in input B, attempt to 
        #                            read next row from input A and skip next step
        #                   (step 2.3) Attempt to read next row from Input B
        #                   (step 2.4) If read attempt was successful, loop (step 2.1)

        self._sort(column_index_left)
        table_right._sort(column_index_right)
        ...
        while True :
            ...
            if idx_left == len(row_left) - 1 or idx_right == len(row_right) - 1: break
    ...
    def _index_nested_loop_join(self, table_right: Table, condition):
        ...
        #                           (step 1) retrieve the indexes (on the primary keys)
        #                           (step 2) ONE loop over right table
        #                           (step 3) retrieve the left index if existing

        left_index = self._load_from_file(f'{self.savedir}/indexes/meta_left_index_index.pkl')
        right_index = self._load_from_file(f'{self.savedir}/indexes/meta_right_index_index.pkl')


        or row_right in table_right.data:
            right_value = row_right[column_index_right]

            left_idx = left_indx[right_value]

            if left_idx is not None : 
                row_left = self.data[left_idx]
                join_table._insert(row_left + row_right)
